...
objfunc <- function(modmat, modparams){
  #INPUT:
  ##modmat- model matrix as produced by using the model.matrix function on the whole formula to be optimized - needed for all objective functions
  ##modparams - named list of all other elements needed for objective function below. Named elements are as follows:
  ####objname - name of objective function to use - needed for all objective functions
  ####momentmatrix - moment matrix. Must be a p x p matrix where p is the number of columns in modmat - only needed for I optimality
  ####referencemat - reference matrix used to calculate squared error between current design and reference matrix - only needed for SSE objective function
  ####priorvals - vector of prior values used for choice based (multinomial logit) optimality criteria
  ####altvect - vector of question set IDs used for choice based (multinomial logit) optimality criteria
  ####matchupframe - choice tournament matrix generated by function gentourneybracket. Used for calculating optimality for tournament based choice optimality criteria
...
bayesinfochoice <- function(bayesmat, modmat, altvect){
  ...
  #function(bayesmat, modmat, altvect, parammean, paramsigma, logdet = sum(log(eigen(paramsigma, symmetric = TRUE, only.values = TRUE)$values))
  #Note - making function compatible with cubature package was attempted but is FAR too slow to be usable. Approach abandoned and random discrete sampling with averaging used instead.
  ...
  #INPUTS:
  ##bayesmat - matrix - matrix of points point at which functionshould be returned. Each row is the evaluation point for that respective variable. Each column will be interpreted as a separate point.
  ##modmat - matrix - model matrix of single choice set to evaluate (produced using model.matrix(formula, design matrix))
  ...
  ##paramsigma - matrix - covariance matrix of coefficient estimates to use for calculating probabilities for the formula passed in modform. Position must match coefficient column positions in modmat
  ##logdet - scalar - log of determinant of covariance matrix. Can be supplied as argument to speed up iterations. Default formula should yield same results as log(det(sigma))
